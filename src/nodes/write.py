...
def get_writer_system_prompt() -> str:
    """
    Generates the specialized system prompt for the Writer subagent.
    """
...
Instructions:
1. Review the input notes, raw research findings, or requirements provided (use `read_file` to load any workspace files referenced in the task).
...
6. Save the final drafted content as a markdown file under the `./workspace` directory (e.g., `workspace/blog_post.md`) using the `write_file` tool.
